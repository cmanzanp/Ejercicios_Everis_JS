...
let ul = document.getElementsByClassName("color-list")[0];
...
let button2 = [];
let mensaje1 = [];
let body = ul.previousSibling.parentNode;

for (let i = 0; i < colorList.length; i++) {
  li[i] = document.createElement("li");
  li[i].classList.add("color-item");
  div1[i] = document.createElement("div");
  div1[i].classList.add("color-name1");
  text1[i] = document.createTextNode("Color: " + colorList[i].colorName);
  div1[i].appendChild(text1[i]);
  div2[i] = document.createElement("div");
  div2[i].classList.add("color-show");
  text2[i] = document.createTextNode("Muestra");
  div2[i].appendChild(text2[i]);
  div2[i].style.backgroundColor = colorList[i].hex;
  li[i].appendChild(div1[i]);
  li[i].appendChild(div2[i]);
  if (i % 2 === 1) {
    li[i].classList.add("color-item--odd");
  }
  button1[i] = document.createElement("button");
  button1[i].addEventListener("click", event => {
    event.stopPropagation();
    if (i === li.length - 1) {
      li[0].style.backgroundColor = colorList[i].hex;
    } else {
      li[i + 1].style.backgroundColor = colorList[i].hex;
    }
  });
  text1[i] = document.createTextNode("Next item color");
  button1[i].appendChild(text1[i]);
  button1[i].classList.add("color-set");
  li[i].appendChild(button1[i]);
  button2[i] = document.createElement("button");
  button2[i].addEventListener("click", event => {
    event.stopPropagation();
    body.style.backgroundColor = colorList[i].hex;
  });
  text2[i] = document.createTextNode("Page color");
  button2[i].appendChild(text2[i]);
  button2[i].classList.add("color-set");
  li[i].appendChild(button2[i]);
...
body.addEventListener("click", alertBody, false);
